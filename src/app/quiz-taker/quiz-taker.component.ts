...
public submit(){
    ...
    //This is actual logic to calculate the score
    this.result.marks = ((Marksmaximum/NumberOfQ))*this.score;//10/2 (5)*4 
    // this.quiz.givenanswers = this.selectedOptions;
    // console.log("VVVVVVVVVVVVVV"+this.selectedOptions);
    if (!this.selectedOptions[this.currentQuestionIndex]) {
      this.selectedOptions[this.currentQuestionIndex] = 
      this.questions[this.currentQuestionIndex].givenAnswer;
      console.log(this.questions[this.currentQuestionIndex].givenAnswer," nnnnnn");
    }
    this.result.answers = this.questions.map((question, index) => {
    // const answersPayload = this.questions.map((question, index) => {
      return {
        quesId: question.quesId,
        givenAnswer: this.selectedOptions[index]
        // givenAnswer: question.givenAnswer

      };
    });

    // this.quizService.submitAnswers(answersPayload).subscribe(
    //   (data: any) => {
...
nextQuestion() {
  if (this.selectedOptions[this.currentQuestionIndex]) 
  {
    this.question.givenAnswer = this.selectedOptions[this.currentQuestionIndex];
...
export class QuizTakerComponent implements OnInit{
  ...
  timer: number = 0;
  timerInterval: any;
  studentsanswers = new Map<number, string>();
// created object here user proprties added here thtat html data it will carry and send to db
constructor(private quizService:QuizService,private router:Router,
  private activatedRoute:ActivatedRoute , private loacalStorage:SessionStorageService,
  private userService:UserService, ){}
...
public submit(){
    ...
    // const mapData ={
    //   studentsanswers :this.studentsanswers
    // }
    // this.quizService.submitAnswers(this.studentsanswers).subscribe(
    //   (data:any)=>{
    //     console.log(data);
    //   }
    // );
...
//   { 
//     this.question.givenAnswer = this.selectedOptions[this.currentQuestionIndex];//
//     // console.log(this.questions[this.currentQuestionIndex].quesId);
//     this.populateAnswers(this.questions[this.currentQuestionIndex].quesId,this.question.givenAnswer);
...
populateAnswers(qId:number,selectedAnswer:string)
  {
    this.studentsanswers.set(qId,selectedAnswer);
